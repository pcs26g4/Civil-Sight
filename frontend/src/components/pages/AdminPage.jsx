...
export default function AdminPage() {
  ...
  const [view, setView] = useState("dashboard"); // Set dashboard as default view
  ...
  const [inspectorComplaintsTab, setInspectorComplaintsTab] = useState("open"); // New state for tabs
  ...
  const viewInspectorComplaints = (inspector) => {
    ...
    setInspectorComplaintsTab("open"); // Reset to open tab when opening modal
  ...
  // Filter complaints based on active tab for inspector complaints modal
            ...
            <div className="modal-body">
              {ticketDetails ? (
                <>
                  ...
                  {(ticketDetails.sub_tickets || []).some(st => st.image_id) && (
                    <div className="image-preview-section">
                      <h3>Image Preview</h3>
                      <div className="image-grid">
                        {(ticketDetails.sub_tickets || []).filter(st => st.image_id).map((st, index) => (
                          <div key={index} className="image-preview-item">
                            <div className="image-container">
                              <img 
                                src={getImageUrl(st.image_id)} 
                                alt={`Complaint ${index + 1}`}
                                className="preview-image"
                                onClick={() => {
                                  setPreviewMedia({
                                    url: getImageUrl(st.image_id),
                              ...
                              {user.role === "INSPECTOR" ? (
                                <>
                                  <button
                                    ...
                                    onClick={() => viewInspectorComplaints(user)}
